...
def render_text_report(metrics: SimulationMetrics) -> str:
    lines = [
        ...
        f"RX2 Successes: {metrics.rx2_successes}",
        f"Duty Cycle Delays: {metrics.duty_cycle_delays}",
        f"Channel Busy Delays: {metrics.channel_busy_delays}",
        f"Average Latency: {metrics.average_latency_seconds:.4f}s",
        f"Total Airtime: {metrics.total_airtime_seconds:.4f}s",
        f"Total Duty Wait: {metrics.total_duty_cycle_wait_seconds:.4f}s",
        f"Total Channel Wait: {metrics.total_channel_busy_wait_seconds:.4f}s",
